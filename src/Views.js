...
Fiber.Views = Fiber.Class.extend({
  ...
  handleRootElement: function($el) {
    ...
    if (! ($el instanceof Fiber.$) && ! _.isString($el))
      $log.errorThrow('`$el` should be a valid jQuery element or selector.');
  ...
  _renderToDestination: function(selector, view) {
    ...
    if (! $destination.length) $log.errorThrow('Destination selector `' + selector + '` is not found in the DOM.');
    if (! (view instanceof Backbone.View)) $log.errorThrow('Cannot render View. Not instance of Backbone.View.');
